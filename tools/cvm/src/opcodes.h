...
void execution(unsigned char opcode, unsigned char *memory, int *pc, long *R, GLFWwindow *window, char *CW, long *SP, unsigned char *stack)
{
	...
	switch(opcode)
	{
		...
		case 0xFA:
			...
			break;
		case 0xFB:
			container = ((memory[R[b[0]]+7] << 56) | (memory[R[b[0]]+6] << 48) | (memory[R[b[0]]+5] << 40) | (memory[R[b[0]]+4]<< 32) | (memory[R[b[0]]+3]<< 24) | (memory[R[b[0]]+2] << 16) | (memory[R[b[0]]+1]<< 8) | memory[R[b[0]]])&0xFFFFFFFFFFFFFFFF;
			printf("%ld", container);
			*pc += 1;
			break;
		case 0xFC:
